...
class CryptoApp
  def run
    # Создаем объекты логики (без сохранения их в инстанс-переменные)
    api = ApiClient.new
    analyzer = Analyzer.new
    portfolio = PortfolioCalculator.new
    chart_drawer = ChartDrawer.new

    # Главное окно (локальная переменная)
    root = TkRoot.new do
    ...
    # Переменные Tk, хранящие выбор пользователя
    ...
    # Получаем холст из ChartDrawer и сохраняем локально
    ...
    theme_btn.command = proc do
      # Читаем текущий цвет, чтобы понять, какая тема включена
    ...
    analyze_btn.command = proc do
      ...
      begin
        # Передаем значения из TkVariables в API
